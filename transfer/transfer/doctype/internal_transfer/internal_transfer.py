...
from transfer.transfer.api import (
    ...
    create_journal_entry as cr_j,
    get_main_account,
    get_profit_account,
    get_currency_remaining_qty,
...
def create_journal_entry(self, temp=False):
    ...
    try:
        ...
        if self.from_type == "Customer":
            ...
            if self.to_type == "Customer" and self.from_type != "Customer":
                ...
                debit_in_account_currency = (
                    self.amount + other_party_profit + our_profit
                )
        ...
        frappe.log_error(frappe.get_traceback(), "Error in Creating Journal Entry")
        frappe.throw(
            f"An error(888) occurred while creating the Journal Entry: {str(e)}"
        )
    ...
    try:
        ...
        if doc.check_tslmfrommain:
            if doc.to_company == frappe.get_cached_doc("Transfer Setting").main_branch:
                ...
                from_account_balance = get_balance_on(
                    get_company_main_account(), date=nowdate()
                )
                if from_account_balance < doc.amount:
                    frappe.throw(
                        f"الرصيد غير كافي في الحساب. الرصيد الحالي: {from_account_balance}, المبلغ المطلوب: {doc.amount}"
                    )
